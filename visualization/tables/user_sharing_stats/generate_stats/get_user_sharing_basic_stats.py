...
class UserSharingStats:
    ...
    def __init__(self, database):
        """ Initializes the UserSharingStats class.
...
class LargeUserSharingStats:
    """ Contains functions to get statistics about how large users share articles. 
    ...
    def __init__(self, database):
        """ Initializes the LargeUserSharingStats class.
        ...
        for user in dist['all-users']:
            ...
            if num_shared > (1.5733 + (2 * 2.1422)):
                self.shares_per_user.append(num_shared)

        for fake_user in dist['fake-users']:
            num_shared = database['users']['counts'][fake_user]['articles']['num-shared']
            if num_shared > (1.373 + (2 * 1.2792)):
                self.shares_per_fake_user.append(num_shared)

        for real_user in dist['real-users']:
            num_shared = database['users']['counts'][real_user]['articles']['num-shared']
            if num_shared > (1.5899 + (2 * 2.7163)):
                self.shares_per_real_user.append(num_shared)
    ...
    def mean(self):
        ...
        Returns:
            `dict`: A dictionary containing the mean values outlined above.
        """
        return {"mean-shares-per-large-user": statistics.mean(self.shares_per_user),
                "mean-shares-per-large-fake-user": statistics.mean(self.shares_per_fake_user),
                "mean-shares-per-large-real-user": statistics.mean(self.shares_per_real_user)}
    ...
    def median(self):
        ...
        Returns:
            `dict`: A dictionary containing the median values outlined above.
        """
        return {"median-shares-per-large-user": statistics.median(self.shares_per_user),
                "median-shares-per-large-fake-user": statistics.median(self.shares_per_fake_user),
                "median-shares-per-large-real-user": statistics.median(self.shares_per_real_user)}
    ...
    def mode(self):
        ...
        Returns:
            `dict`: A dictionary containing the mode values outlined above.
        """
        return {"mode-shares-per-large-user": statistics.mode(self.shares_per_user),
                "mode-shares-per-large-fake-user": statistics.mode(self.shares_per_fake_user),
                "mode-shares-per-large-real-user": statistics.mode(self.shares_per_real_user)}
    ...
    def stdev(self):
        ...
        Returns:
            `dict`: A dictionary containing the variance values outlined above.
        """
        return {"stdev-shares-per-large-user": statistics.stdev(self.shares_per_user),
                "stdev-shares-per-large-fake-user": statistics.stdev(self.shares_per_fake_user),
                "stdev-shares-per-large-real-user": statistics.stdev(self.shares_per_real_user)}
    ...
    def range(self):
        ...
        Returns:
            `dict`: A dictionary containing the range values outlined above.
        """
        return {"range-shares-per-large-user": max(self.shares_per_user) -
                min(self.shares_per_user),
                "range-shares-per-large-fake-user": max(self.shares_per_fake_user) -
                min(self.shares_per_fake_user),
                "range-shares-per-large-real-user": max(self.shares_per_real_user) -
                min(self.shares_per_real_user)}
    ...
    def iqr(self):
        ...
        Returns:
            `dict`: A dictionary containing the IQR values outlined above.
        """
        return {"iqr-shares-per-large-user": statistics.quantiles(self.shares_per_user, n=4)[2] -
                statistics.quantiles(self.shares_per_user, n=4)[0],
                "iqr-shares-per-large-fake-user": statistics.quantiles(self.shares_per_fake_user,
                                                                       n=4)[2] -
                statistics.quantiles(self.shares_per_fake_user, n=4)[0],
                "iqr-shares-per-large-real-user": statistics.quantiles(self.shares_per_real_user,
                                                                       n=4)[2] -
                statistics.quantiles(self.shares_per_real_user, n=4)[0]}
    ...
    def skew(self):
        ...
        Returns:
            `dict`: A dictionary containing the skew values outlined above.
        """
        return {"skew-shares-per-large-user": stats.skew(self.shares_per_user),
                "skew-shares-per-large-fake-user": stats.skew(self.shares_per_fake_user),
                "skew-shares-per-large-real-user": stats.skew(self.shares_per_real_user)}
    ...
    def kurtosis(self):
        ...
        Returns:
            `dict`: A dictionary containing the kurtosis values outlined above.
        """
        return {"kurtosis-shares-per-large-user": stats.kurtosis(self.shares_per_user),
                "kurtosis-shares-per-large-fake-user": stats.kurtosis(self.shares_per_fake_user),
                "kurtosis-shares-per-large-real-user": stats.kurtosis(self.shares_per_real_user)}
