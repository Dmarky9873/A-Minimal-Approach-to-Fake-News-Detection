"""

    Authors: Daniel Markusson


"""

from visualization.ecdfs.p_length_vs_type_of_article.generate_stats.get_length_articles_lists \
    import get_length_articles_lists
from visualization.ecdfs.create_ecdf import create_comparitive_ecdf


def create_p_fake_vs_length_ecdf():
    """
        Creates the ECDF of the probability to have a certain number of authors based on the type
        of article.
    """
    all_articles, fake_articles, real_articles = get_length_articles_lists()

    combined_list = []
    labels = []
    for num_authors in all_articles:
        combined_list.append(num_authors)
        labels.append("Combined")
    for num_authors in fake_articles:
        combined_list.append(num_authors)
        labels.append("Fake")
    for num_authors in real_articles:
        combined_list.append(num_authors)
        labels.append("Real")

    create_comparitive_ecdf(combined_list, labels, ["Length of Article (in Characters)",
                                                    "Type of Article"],
                            "Type of Article",
                            "Length of Article (in Characters)",
                            "Proportion of Articles with Different Lengths Depending on Type of Article",
                            "p_length_vs_type_of_article.png")


def main():
    """ Main function for the script.
    """
    create_p_fake_vs_length_ecdf()


if __name__ == "__main__":
    main()
